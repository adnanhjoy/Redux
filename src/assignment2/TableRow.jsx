...
import { cancelBooking } from '../redux/bookingFlight/actions';

const TableRow = ({ flight }) => {
    const dispatch = useDispatch();
    const { from, to, date, guests, ticketClass } = flight || {};

    const handleDelete = (dFlight) => {
        dispatch(cancelBooking(dFlight))
    }
    return (
        <tr className="lws-bookedTable text-black">
            ...
            <td className="px-6 py-4 text-center">
                <div className="flex justify-center gap-4">
                    <button onClick={() => handleDelete(flight)} className="lws-remove">
